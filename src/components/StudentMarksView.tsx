  ...
  const loadMarks = async () => {
    try {
      ...
      const { data: subjectsData, error: subjectsError } = await supabase
        .from('subjects')
        .select('*')
                // Load all marks for the student. If a classId is provided, prefer filtering by it.
                ...
                if (classId && String(classId).trim() !== '') {
                  marksQuery = (marksQuery as any).eq('class_id', classId);
                }
      ...
      setMarksHistory(historyData || []);
                // If we didn't find any marks when filtered by class, fall back to querying by student only
                let effectiveMarks = marksData || [];
      ...
      const StudentMarksView: React.FC<StudentMarksViewProps> = ({
        ...
        const loadMarks = async () => {
          try {
            ...
            // Build base query to fetch marks with subject and class relation
            ...
            // If a classId is provided, prefer filtering by it
            if (classId && String(classId).trim() !== '') {
                          ...
                          const StudentMarksView: React.FC<StudentMarksViewProps> = ({
                            ...
                            const loadMarks = async () => {
                              try {
                                ...
                                // Load marks history (recent updates)
                                ...
                                // Calculate latest exam percentage from effectiveMarks
                                ...
                                console.error('Error loading marks:', error?.message || error);
                              } finally {
                                setLoading(false);
                              }
                            };
