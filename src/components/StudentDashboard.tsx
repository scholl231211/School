...
import { BookOpen, TrendingUp, User, Mail, Bot, Star, GraduationCap, Clock } from 'lucide-react';
...
const StudentDashboard: React.FC = () => {
  ...
  const getLatestExamType = (examTypes: string[]): string | null => {
    ...
    return validExams.sort((a, b) => EXAM_ORDER.indexOf(b) - EXAM_ORDER.indexOf(a))[0];
  };

  // ✅ FIXED: removed invalid useCallback inside useEffect
  useEffect(() => {
    ...
    try {
      ...
      if (studentData.id) {
        ...
        if (allMarks && allMarks.length > 0) {
          ...
          const totalMarks = latestExamMarks.reduce(
            (sum, mark) => sum + Number(mark.total_marks || 0),
            0
          );
          const obtainedMarks = latestExamMarks.reduce(
          ...
          if (totalMarks > 0) {
            ...
            setLatestExamPercentage(Math.round((obtainedMarks / totalMarks) * 10000) / 100);
          }
          ...
          const obtainedAll = allMarks.reduce(
            (sum, mark) => sum + Number(mark.marks_obtained || 0),
            0
          );

          if (totalAll > 0) {
            setOverallPercentage(Math.round((obtainedAll / totalAll) * 10000) / 100);
          }
        } else {
          setLatestExamType('');
  ...
  // ✅ Attendance calculation moved to top level only once
  ...
  // ✅ Effect to update attendance percentage when student changes
  useEffect(() => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="fixed bottom-6 right-6 flex flex-col gap-4">
          ...
          <button
            ...
            className="bg-yellow-500 hover:bg-yellow-600 text-white p-4 rounded-full shadow-lg transition-all duration-300"
          >
        ...
        <AnimatePresence>
          {showAIHelper && (
            <AIHelper onClose={() => setShowAIHelper(false)} studentName={student.name} />
          )}
          {showRatingModal && (
            <RatingModal onClose={() => setShowRatingModal(false)} studentId={student.id} />
          )}
        </AnimatePresence>
      </div>
    </div>
